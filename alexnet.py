...
def model():
    ...
    with tf.name_scope('conv1') as scope:
        ...
        pre_activation_conv1 = tf.nn.bias_add(conv, biases)
        conv1 = tf.nn.relu(pre_activation_conv1, name=scope)  # 这里返回的是一个tensor（一个张量类）
    ...
    with tf.name_scope('conv2') as scope:
        ...
        pre_activation_conv2 = tf.nn.bias_add(conv, biases)
        conv2 = tf.nn.relu(pre_activation_conv2, name=scope)
